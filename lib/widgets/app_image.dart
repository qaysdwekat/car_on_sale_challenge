...
class AppImage extends StatelessWidget {
  ...
  Widget loadLocalAsset(String imageUrl) {
    ...
    if (Platform.isAndroid) {
      ...
      if (file.existsSync()) {
        return Image.file(File(imageUrl), width: width, height: height, fit: fit, color: color);
      }
    }

    if (imageUrl.startsWith('assets')) {
      return Image.asset(imageUrl, width: width, height: height, fit: fit, color: color, package: package);
    }
    return placeholderWidget ?? ((placeholder != null) ? loadLocalAsset(placeholder!) : const SizedBox());
  }
}
